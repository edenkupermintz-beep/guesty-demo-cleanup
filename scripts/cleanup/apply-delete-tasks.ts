/**
 * Apply tasks-delete-plan.json: DELETE each mutation task.
 * Resumable via tasks-delete-results.json (skips already-ok taskIds).
 *
 * Usage:
 *   npm run cleanup:delete-tasks                 # dry-run summary
 *   npm run cleanup:delete-tasks -- --apply      # mutate
...
function parseArgs(argv: string[]) {
  ...
  return {
    ...
    planPath: resolve(
      process.cwd(),
      argv.find((a, i) => argv[i - 1] === "--plan") ?? "tasks-delete-plan.json",
    ),
    resultsPath: resolve(process.cwd(), "tasks-delete-results.json"),
    delayMs: numFlag("--delay-ms", 100),
    concurrency: Math.max(1, numFlag("--concurrency", 2)),
    limit: (() => {
...
async function deleteWithRetry(
  ...
  for (;;) {
    ...
    try {
      ...
      // Already gone counts as success for cleanup
...
async function main() {
  ...
  let mutations = plan.mutations.filter((m) => m.op === "delete_task" && m.taskId);
  ...
  if (!args.apply) {
    console.log("Dry-run only. Re-run with --apply to DELETE tasks.");
    return;
  }
  ...
  async function worker() {
    for (;;) {
      ...
      const m = pending[i]!;
      try {
        await deleteWithRetry(client, m.taskId);
